...
//const CaptionFromSeed = dynamic(() => import("../caption"), { ssr: false });
...
export default function ShaderCanvas() {
  ...
  const [generationState, setGenerationState] = useState<PageState>(
    PageState.INITIAL
  );
  ...
  useEffect(() => {
    if (dataManager?.globals && sculptureId && selected) {
      ...
      if (savedDataString) {
        if (sculptureId == "infinite") {
          localStorage.setItem(sculptureId, perf);
        } else {
    ...
    (time: number, delta: number, uuid: string | null, orbit: number) => {
      if (
        ...
        if (
          ...
          if (dataManager?.globals) {
            dataManager.globals.camHAngle +=
  ...
  const resetTimer = useCallback((time: number, delta: number) => {
    ...
    if (timeRef.current <= 0) {
      setGenerationState(PageState.RESET);
    } else {
      requestAnimationFrame((newTime: number) => {
  ...
  useEffect(() => {
    ...
    if (dataManager) {
      if (generationState === PageState.INITIAL) {
        if (sculptureId) {
          const savedDataString = localStorage.getItem(sculptureId as string);
          if (savedDataString && sculptureId != "infinite") {
            const savedData = JSON.parse(savedDataString) as UiData;
            dataManager.setData(savedData);
            setSelected(dataManager.globals.perf);
            dataManager.updateShader();
            setGenerationState(PageState.COMPLETE);
          } else {
            defaults.generationSettings.sculptureId = sculptureId as string;
            dataManager.setData(defaults as UiData);
            dataManager.materials[1].color.r = Math.random();
            dataManager.shapes[0].pos = {
              x: Math.random(),
              y: Math.random(),
              z: Math.random(),
            };
      ...
      } else if (generationState == PageState.DATA_LOADED) {
        if (compiledRef.current) {
          ...
          if (sculptureId == "infinite" && savedData) {
            ...
            setGenerationState(PageState.TESTING_PERFORMANCE);
          }
        }
      } else if (generationState == PageState.TESTING_PERFORMANCE) {
      ...
      } else if (generationState == PageState.READY_TO_GENERATE) {
        setCompiled(false);
        setGenerationState(PageState.GENERATING_BASE);
        dataManager.globals.camVAngle = 1.26;
        setGlobalsUpdated((prev) => prev + 1);
      } else if (generationState == PageState.GENERATING_BASE) {
        if (sculptureId == "infinite") {
          dataManager.generationSettings.sculptureId = uuidv4();
          const newParams = new URLSearchParams();
          newParams.set("id", dataManager.generationSettings.sculptureId);
          routerRef.current.push(`?${newParams.toString()}`);
        }
        dataManager.generateBaseSculpture().then(() => {
          setGenerationState(PageState.BASE_GENERATED);
        });
      } else if (generationState == PageState.BASE_GENERATED) {
        if (upgrades > 0) {
          setGenerationState(PageState.GENERATING_UPGRADES);
        } else if (compiledRef.current) {
          setGenerationState(PageState.COMPLETE);
        }
      } else if (generationState == PageState.GENERATING_UPGRADES) {
        (async () => {
          for (let i = 0; i < upgrades; i++) {
            await dataManager.upgradeSculpture();
          }
          setGenerationState(PageState.UPGRADES_GENERATED);
        })();
      } else if (generationState == PageState.UPGRADES_GENERATED) {
        if (compiledRef.current) {
          setGenerationState(PageState.COMPLETE);
        }
      } else if (generationState == PageState.COMPLETE) {
        if (sculptureId != "infinite") {
          localStorage.setItem(
            sculptureId as string,
            JSON.stringify(dataManager.getData())
          ...
          localStorage.setItem(sculptureId, dataManager.globals.perf);
        }
        if (raf.current == null) {
      ...
      } else if (generationState == PageState.RESET) {
        setTimeout(() => {
          requestAnimationFrame((time) => doDot(time));
          setGenerationState(PageState.READY_TO_GENERATE);
        }, 1000);
          ...
          <div
            className={`flex flex-row w-[640px] text-[9px] uppercase text-gray-400 tracking-widest underline underline-offset-2 justify-around self-center mb-2
              transition-[opacity] duration-1000 ${clsx({
                "opacity-0":
                  generationState != PageState.COMPLETE || !showInterface,
              })}`}
          >
            <div
              ...
              onClick={() => {
                ...
                if (dataManager?.globals) {
                  dataManager.globals.camHAngle = 0.0;
                  dataManager.globals.camVAngle = 1.3;
                  dataManager.globals.camDist = 0.9;
                }
                setGlobalsUpdated((prev) => prev + 1);
                raf.current = null;
                setOrbit(0);
            ...
            <div
              ...
              onClick={() => {
                throttleInteract(interact, 100);
                if (dataManager?.globals) {
                  dataManager.globals.camHAngle = 2.1;
                  dataManager.globals.camVAngle = 1.3;
                  dataManager.globals.camDist = 1.2;
                }
                setGlobalsUpdated((prev) => prev + 1);
                raf.current = null;
                setOrbit(0);
            ...
            <div
              ...
              onClick={() => {
                throttleInteract(interact, 100);
                if (dataManager?.globals) {
                  dataManager.globals.camHAngle = 4.2;
                  dataManager.globals.camVAngle = 1.0;
                  dataManager.globals.camDist = 0.7;
                }
                setGlobalsUpdated((prev) => prev + 1);
                raf.current = null;
                setOrbit(0);
            ...
            <div
              ...
              onClick={() => {
                throttleInteract(interact, 100);
                if (dataManager?.globals) {
                  dataManager.globals.camHAngle = -0.5;
                  dataManager.globals.camVAngle = 0.5;
                  dataManager.globals.camDist = 0.8;
                }
                setGlobalsUpdated((prev) => prev + 1);
                raf.current = null;
                setOrbit(0);
            ...
            <div className="w-[640px] flex flex-col h-full">
              <div className="h-[360px] relative">
                ...
                >
                  <div>
                    <span>
                      {generationState in pageLabels
                        ? pageLabels[generationState as keyof typeof pageLabels]
                        : compiled
                        ? ""
                        : "Compiling"}
                    </span>

                    {(generationState in pageLabels || !compiled) && (
                      <>
