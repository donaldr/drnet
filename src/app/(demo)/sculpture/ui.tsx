...
export class RaymarchingUI {
  ...
  build(data?: UiData) {
    ...
    this.dataManager.updateShader();

    this.pane.addButton({ title: "Export" }).on("click", () => {
      const state: UiData = this.dataManager.getData();
  ...
  save() {
    this.dataManager.updateShader();
    localStorage.setItem("uidata", JSON.stringify(this.dataManager.getData()));
  }
  ...
  setupGenerationFolder() {
    ...
    f.addBinding(this.dataManager.generationSettings, "sculptureId", {
      label: "Sculpture ID",
    });
    const genIdBtn = f.addButton({ title: "Generate ID" });
    genIdBtn.on("click", () => {
      this.dataManager.generationSettings.sculptureId = uuidv4();
      this.pane.refresh();
      this.save();
    });
    f.addBinding(this.dataManager.generationSettings, "baseShapeCount", {
      label: "Base Shape Count",
      min: 1,
      max: 10,
      step: 1,
    });
    f.addBinding(this.dataManager.generationSettings, "upgradeShapeCount", {
  ...
  setupGlobalFolder() {
    ...
    f.addBinding(this.dataManager.globals, "camTgt", {
      label: "Camera Target",
      step: 0.01,
    }).on("change", () => {
    ...
    f.addBinding(this.dataManager.globals, "boundingBoxPos", {
    ...
    f.addBinding(this.dataManager.globals, "boundingBoxDims", {
    ...
    f.addBinding(this.dataManager.globals, "showDebug", {
      label: "Show Debug",
    }).on("change", () => {
      if (this.dataManager.setGlobalsUpdated) {
        this.dataManager.setGlobalsUpdated((prev) => prev + 1);
    ...
    f.addBinding(this.dataManager.globals, "debugMode", {
      label: "Debug Mode",
    ...
    f.addBinding(this.dataManager.globals, "mapScale", {
      min: 0.0,
      max: 2.0,
      step: 0.1,
      label: "Heatmap Scale",
    ...
    f.addBinding(this.dataManager.globals, "showBoxes", {
    ...
    f.addBinding(this.dataManager.globals, "showBoundingBox", {
  ...
  setupPerformanceFolder() {
    ...
    for (let i = 0; i < 3; i++) {
      const key = mode[i];
      const p = this.dataManager.performanceSettings[key];
      tabs.pages[i]
  ...
  addColorPalette(f: FolderApi, palette: ColorPalette) {
    ...
    paletteFolder.addButton({ title: "Remove Palette" }).on("click", () => {
      this.dataManager.colorPalettes.splice(
        this.dataManager.colorPalettes.findIndex(
  ...
  setupColorPalettesFolder() {
    ...
    this.dataManager.colorPalettes.forEach((palette) => {
    ...
    f.addButton({ title: "Add Palette" }).on("click", () => {
      ...
      this.dataManager.colorPalettes.push(newPalette);
  ...
  addShapeMaterialRules(
    ...
    return createMultiSelectBlade(
      f,
      this.dataManager.materialRules
        .slice(0)
  ...
  addShapeRule(shapeRule: ShapeGeneratorDetails) {
    ...
    rmBtn.on("click", () => {
      if (this.dataManager.shapes.length <= 1) return;
      this.dataManager.shapes.splice(
        this.dataManager.shapes.findIndex((el) => el.uuid == shapeRule.uuid),
        1
      );
  ...
  setupShapeRulesFolder() {
    ...
    addBtn.on("click", () => {
      ...
      this.dataManager.shapeRules.push(shapeRule);
    ...
    this.dataManager.shapeRules.forEach((shapeRule, index) => {
      if (!shapeRule.uuid) {
        if (index == 0) {
      ...
      if (child instanceof FolderApi) {
        ...
        if (foundIndex >= 0) {
          this.addShapeMaterialRules(
            child,
            this.dataManager.shapeRules[index],
            foundIndex
          );
        }
        index++;
  ...
  addMaterialMultiSelect(
    ...
    return createMultiSelectBlade(
      f,
      this.dataManager.colorPalettes
        .slice(0)
  ...
  addMaterialRule(mat: MaterialGeneratorDetails) {
    ...
    if (mat.uuid != "FLOOR") {
      const rmBtn = f.addButton({ title: "Remove" });
      rmBtn.on("click", () => {
        this.dataManager.materialRules.splice(
          this.dataManager.materialRules.findIndex((el) => el.uuid == mat.uuid),
          1
        );
  ...
  updateMaterialRules() {
    this.materialRulesFolder?.children.forEach((child, childIndex) => {
      if (child instanceof FolderApi) {
        ...
        const mat = this.dataManager.materialRules[childIndex - 1]; // -1 because first child is the add button
  ...
  setupMaterialRulesFolder() {
    ...
    addBtn.on("click", () => {
      ...
      this.dataManager.materialRules.push(materialRule);
      this.updateShapeMaterialRules();
      this.pane!.refresh();
      this.save();
    });

    this.dataManager.materialRules.forEach((mat, index) => {
      if (!mat.uuid) {
        if (index == 0) {
          mat.uuid = "FLOOR";
        } else if (index == 1) {
  ...
  setupFloorFolder() {
    ...
    f.addBinding(this.dataManager.materials[0], "color", {
      label: "Color",
      color: { type: "float" },
    }).on("change", () => {
      this.dataManager.setMaterialsUpdated!((prev) => prev + 1);
    });
    f.addBinding(this.dataManager.materials[0], "kd", {
      min: 0,
      max: 100,
      step: 0.01,
      label: "Diffuse Strength",
    ...
    f.addBinding(this.dataManager.materials[0], "reflectivity", {
      min: 0,
      max: 1,
      step: 0.01,
      label: "Reflectivity",
    ...
    this.dataManager.materials[0].intRef =
      this.dataManager.materials[0].intRef ?? false;
    f.addBinding(this.dataManager.materials[0], "intRef", {
      label: "Internal Reflection",
    }).on("change", () => {
      this.dataManager.setMaterialsUpdated!((prev) => prev + 1);
    });
    f.addBinding(this.dataManager.materials[0], "roughness", {
      min: 0,
      max: 1,
      step: 0.01,
      label: "Roughness",
    ...
    this.dataManager.materials[0].reflectRoughness =
      this.dataManager.materials[0].reflectRoughness ?? 0.0;
    f.addBinding(this.dataManager.materials[0], "reflectRoughness", {
      min: 0,
      max: 1,
      step: 0.01,
      label: "Reflect Roughness",
    ...
    this.dataManager.materials[0].refractRoughness =
      this.dataManager.materials[0].refractRoughness ?? 0.0;
    f.addBinding(this.dataManager.materials[0], "refractRoughness", {
      min: 0,
      max: 1,
      step: 0.01,
      label: "Refract Roughness",
    ...
    this.dataManager.materials[0].surfaceBlur =
      this.dataManager.materials[0].surfaceBlur ?? 0.0;
    f.addBinding(this.dataManager.materials[0], "surfaceBlur", {
      min: 0,
      max: 1,
      step: 0.01,
      label: "Surface Blur",
    ...
    f.addBinding(this.dataManager.materials[0], "metallic", {
      min: 0,
      max: 1,
      step: 0.01,
      label: "Metallic",
    ...
    if (light.uuid !== "DEFAULT") {
      ...
      rmBtn.on("click", () => {
        if (
          ...
          this.dataManager.lights.length <= 1
        )
          return;
        this.dataManager.lights.splice(
          this.dataManager.lights.findIndex((el) => el.uuid == light.uuid),
          1
        );
  ...
  setupLightsFolder() {
    ...
    addBtn.on("click", () => {
      const hasOmni = this.dataManager.lights.some(
      ...
      light.uuid = uuidv4();
      this.addLight(light);
      this.dataManager.lights.push(light);
      this.pane.refresh();
      this.save();
    });

    this.dataManager.lights.forEach((light, index) => {
      if (!light.uuid) {
        if (index == 0) {
          light.uuid = "DEFAULT";
        } else {
  ...
  setupShapesFolder() {
    ...
    this.dataManager.shapes.forEach((shape) => {
      shape.uuid = uuidv4();
      this.addShape(shape);
    });
  }
