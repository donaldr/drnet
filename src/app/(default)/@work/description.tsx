      ...
      {paragraphs.map((paragraph: string, index: number) => (
        ...
        <SplitText
          ...
          WordWrapper={({ countIndex, children }: any) => {
            const i = Math.floor(
              seedrandom((countIndex * seedRef.current).toString())() * 6
            );
            const itemClasses = clsx({
              ...
              "inline-block opacity-0 group-[.enter]/action:opacity-100 translate-y-[10px] group-[.enter]/action:translate-y-0 transition-[transform,opacity] duration-500":
                true,
              "text-[var(--dark)]": theme == "light",
              "text-[var(--light)]": theme == "dark",
            });
            return <span className={itemClasses}>{children}</span>;
          }}
        >
          {paragraph}
        </SplitText>
